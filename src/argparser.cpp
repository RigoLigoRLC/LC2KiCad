...
namespace lc2kicad
{
  ...
  {
    int endpos = -1;
    bool warnCompatibility = false, noDoubleDash = false;
    ...
    {
      endpos == -1 ? endpos = argc , noDoubleDash = true : 0;
      for(int i = 1; i < endpos; i++)
      {
        ...
        if(noDoubleDash) //Not matching -h -v -c and there's no "--", then treat as filename
          ret.filenames.push_back(argv[i]);
      }
    }
    
    if(!noDoubleDash) //If there is "--" to split args then do following else just return ret
      if(endpos != argc) //If endpos doesn't equal to argc then there are files, just push them back
        for(int i = endpos + 1; i < argc; i++)
          ret.filenames.push_back(argv[i]);
      else if(!ret.invokeHelp && !ret.invokeVersionInfo)
      {
